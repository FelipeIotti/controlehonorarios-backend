  ...
  async execute(): Promise<IGeneralDTO[]> {
    ...
    const newGeneral = lawyers.map(lawyers => {
      ...
      fees.map(fees =>{
        ...
        if(fees.lawyers=== lawyers.name && fees.status==='Concluído' && dayjs(fees.endDate).get('month')=== 0){
          newGeneral.january = newGeneral.january + 1;
        }

        if(fees.lawyers=== lawyers.name && fees.status==='Concluído' && dayjs(fees.endDate).get('month')=== 1){
          newGeneral.february = newGeneral.february + 1;
        }

        if(fees.lawyers=== lawyers.name && fees.status==='Concluído' && dayjs(fees.endDate).get('month')=== 2){
          newGeneral.march = newGeneral.march + 1;
        }

        if(fees.lawyers=== lawyers.name && fees.status==='Concluído' && dayjs(fees.endDate).get('month')=== 3){
          newGeneral.april = newGeneral.april + 1;
        }

        if(fees.lawyers=== lawyers.name && fees.status==='Concluído' && dayjs(fees.endDate).get('month')=== 4){
          newGeneral.may = newGeneral.may + 1;
        }

        if(fees.lawyers=== lawyers.name && fees.status==='Concluído' && dayjs(fees.endDate).get('month')=== 5){
          newGeneral.june = newGeneral.june + 1;
        }

        if(fees.lawyers=== lawyers.name && fees.status==='Concluído' && dayjs(fees.endDate).get('month')=== 6){
          newGeneral.july = newGeneral.july + 1;
        }

        if(fees.lawyers=== lawyers.name && fees.status==='Concluído' && dayjs(fees.endDate).get('month')=== 7){
          newGeneral.august = newGeneral.august + 1;
        }

        if(fees.lawyers=== lawyers.name && fees.status==='Concluído' && dayjs(fees.endDate).get('month')=== 8){
          newGeneral.september = newGeneral.september + 1;
        }

        if(fees.lawyers=== lawyers.name && fees.status==='Concluído' && dayjs(fees.endDate).get('month')=== 9){
          newGeneral.october = newGeneral.october + 1;
        }

        if(fees.lawyers=== lawyers.name && fees.status==='Concluído' && dayjs(fees.endDate).get('month')=== 10){
          newGeneral.november = newGeneral.november + 1;
        }

        if(fees.lawyers=== lawyers.name && fees.status==='Concluído' && dayjs(fees.endDate).get('month')=== 11){
